...
class Colors:
    ...
    def __init__(self):
        ...
        hexs = ('FFFFFF', 'FFFFFF', 'FFFFFF','FFFFFF','FFFFFF','FFFFFF')
...
class Annotator:
    ...
    def masks(self, masks, labels,colors, im_gpu, alpha=1, retina_masks=False):
        """Plot masks at once.
        Args:
        ...
        for index, label in enumerate(labels):
          if(label == 0): 
            person_color.append(colors[index]) 
        person_color = torch.stack(person_color, 0)
        print(f'person_color : {person_color,person_color.shape}')

        person_masks = person_masks.unsqueeze(3)  # shape(n,h,w,1)
        masks_color = person_masks * (person_color * alpha)  # shape(n,h,w,3)
        #print(f'masks : ${masks}')
        inv_alph_masks = (1 - person_masks * alpha).cumprod(0)  # shape(n,h,w,1). #masks * alpha
        ...
        print(f"person_masks : {person_masks.shape}")
        
        mask_img = im_mask if retina_masks else scale_image(mask_img.shape, im_mask, self.im.shape)
        if self.pil:
            # convert im back to PIL and update draw
            self.fromarray(mask_img)
        
        mask_binary_img = mask_img.copy()
        ...
        json_mask_data = {}
        count = 0 
        black_contour_img = self.im.copy()
        black_contour_img[:,:,:] = 0
